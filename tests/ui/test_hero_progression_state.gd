...
func _init() -> void:
	...
	_assert(state.toggle_skill_loadout(&"thorn_orb").ok, "unequip learned skill")
	_assert(not state.loadout_snapshot().has(&"thorn_orb"), "loadout removal")
	_assert(state.toggle_skill_loadout(&"thorn_orb").ok, "reequip learned skill")
	_assert(state.upgrade_skill(&"spirit_link").ok, "level-gated skill upgrade")
	_assert(not state.upgrade_skill(&"thorn_orb").ok, "level 40 skill gate")

	var hp_before := _stat(state, &"max_hp")
	var vitality_rank := _talent_rank(state, &"t_vit")
	_assert(state.set_talent_rank(&"t_vit", vitality_rank - 1).ok, "dependency-safe talent refund")
	_assert(_stat(state, &"max_hp") < hp_before, "talent refreshes calculated stats")
	_assert(state.set_talent_rank(&"t_vit", vitality_rank).ok, "talent rank restore")
	for rank in range(4, -1, -1):
		_assert(state.set_talent_rank(&"t_pow", rank).ok, "refund alternate prerequisite parent")
	_assert(state.set_talent_rank(&"t_vit", 2).ok, "refund vitality to rank two")
	_assert(state.set_talent_rank(&"t_vit", 1).ok, "refund vitality to rank one")
	_assert(not state.set_talent_rank(&"t_vit", 0).ok, "reject refund required by learned child")

	var power_before := _stat(state, &"power")
	_assert(state.item_action(&"oracle_signet") == &"replace", "same-slot replacement action")
	_assert(state.equip_item(&"oracle_signet").ok, "replace equipped ring")
	...
	_assert(state.unequip_item(&"oracle_signet").ok, "unequip item")
	_assert(state.item_action(&"ember_signet") == &"equip", "empty-slot equip action")
	_assert(state.equip_item(&"ember_signet").ok, "equip item")
	quit(0)
