...
class PointsHeader extends Control:
	...
	func _draw() -> void:
		...
		draw_string(UIFonts.rajdhani_bold(), Vector2(size.x - 170, 17), "%d remaining · %d/%d" % [remaining, spent, HeroData.TALENT_POINTS_TOTAL], HORIZONTAL_ALIGNMENT_RIGHT, 158, 9, Color("#AA44FF") if remaining > 0 else Color("#FF7733"))
		...
		draw_rect(Rect2(track.position, Vector2(track.size.x * clampf(float(spent) / HeroData.TALENT_POINTS_TOTAL, 0, 1), track.size.y)), Color("#AA44FF"))
		draw_string(UIFonts.rajdhani_semibold(), Vector2(12, 41), "Ranks: %d/200 · 40 Nodes · Tap node to upgrade" % spent, HORIZONTAL_ALIGNMENT_LEFT, -1, 7, Color("#3A2858"))
		draw_string(UIFonts.rajdhani_semibold(), Vector2(size.x - 110, 41), "Max 99 TP at Lv.99", HORIZONTAL_ALIGNMENT_RIGHT, 98, 7, Color("#3A2858"))
...
class SectionButton extends Control:
	...
	func _draw() -> void:
		...
		var status := "%d pts" % points if unlocked else "LOCK · Lv.%d" % meta["unlock"]
...
class ConstellationGraph extends Control:
	...
	func _draw_node(index: int, scale_value: float) -> void:
		...
		var radius := (21.0 if keystone else 14.0) * scale_value; var has_rank: bool = talent["rank"] > 0
		...
		draw_arc(center, radius, 0, TAU, 28, color if has_rank else Color("#2A1845", 0.45), (1.6 if keystone else 1.1) * scale_value, true)
		IconDraw.draw_item_icon(self, talent["icon"], Rect2(center - Vector2.ONE * radius * 0.45, Vector2.ONE * radius * 0.9), color if has_rank or keystone else Color("#4A3870"))
		...
		draw_string(UIFonts.rajdhani_bold(), center + Vector2(-45 * scale_value, radius + 10 * scale_value), short_name, HORIZONTAL_ALIGNMENT_CENTER, 90 * scale_value, maxi(6, int(7 * scale_value)), color if has_rank else Color("#3A2858"))
...
class LockedView extends Control:
	...
	func _draw() -> void:
		...
		draw_string(UIFonts.cinzel_bold(), center + Vector2(-110, 42), "REQUIRES LV.%d" % meta["unlock"], HORIZONTAL_ALIGNMENT_CENTER, 220, 11, Color("#2A1845"))
		draw_string(UIFonts.rajdhani_medium(), center + Vector2(-110, 70), "%s grants %d talent nodes and up to %d ranks." % [meta["label"].capitalize(), meta["nodes"], meta["nodes"] * 5], HORIZONTAL_ALIGNMENT_CENTER, 220, 8, Color("#1E1438"))
...
class DetailPanel extends Control:
	...
	func _ready() -> void:
		var minus := _button("−", Color("#FF7733"), talent["rank"] > 0); minus.anchor_left = 1; minus.anchor_right = 1; minus.offset_left = -72; minus.offset_right = -42; minus.offset_top = 50; minus.offset_bottom = 76; minus.pressed.connect(func() -> void: decrease_requested.emit(talent["id"])); add_child(minus)
		var plus := _button("+", color, can_increase); plus.anchor_left = 1; plus.anchor_right = 1; plus.offset_left = -38; plus.offset_right = -8; plus.offset_top = 50; plus.offset_bottom = 76; plus.pressed.connect(func() -> void: increase_requested.emit(talent["id"])); add_child(plus)
	...
	func _draw() -> void:
		...
		for i in range(talent["max"]): draw_rect(Rect2(12 + i * (segment + 2), 58, segment, 5), color if i < talent["rank"] else Color("#1E1438"))
		draw_string(UIFonts.rajdhani_bold(), Vector2(size.x - 108, 65), "%d/%d" % [talent["rank"], talent["max"]], HORIZONTAL_ALIGNMENT_CENTER, 32, 9, color if talent["rank"] > 0 else Color("#3A2858"))
...
func _ready() -> void:
	clip_contents = true
	for talent in HeroData.constellation_talents(): _talents.append(talent.duplicate(true))
...
func _rebuild() -> void:
	...
	queue_redraw()
	var points := PointsHeader.new(); points.position = Vector2.ZERO; points.size = Vector2(size.x,48); points.spent = _spent(); points.remaining = HeroData.TALENT_POINTS_TOTAL - points.spent; add_child(points)
	var gap := 6.0; var tab_w := (size.x - 16 - gap*3) / 4.0
	for i in range(SECTION_ORDER.size()):
		var id := SECTION_ORDER[i]; var button := SectionButton.new(); button.section_id = id; button.meta = HeroData.talent_sections()[id]; button.active = id == _active_section; button.points = _section_points(id)
	...
	var detail_height := 84.0 if not _selected_id.is_empty() else 0.0
	var graph_rect := Rect2(6,92,size.x-12,maxf(0,size.y-98-detail_height))
	if HeroData.PROGRESSION_LEVEL < meta["unlock"]:
		...
		var graph := ConstellationGraph.new(); graph.meta = meta; graph.talents = _section_talents(_active_section); graph.positions = PATH_POS if meta["nodes"] == 12 else SPEC_POS; graph.edges = PATH_EDGES if meta["nodes"] == 12 else SPEC_EDGES; graph.selected_id = _selected_id; graph.position = graph_rect.position; graph.size = graph_rect.size; graph.node_pressed.connect(_select_node); add_child(graph)
	if not _selected_id.is_empty():
		var talent := _find_talent(_selected_id)
		if not talent.is_empty():
			var detail := DetailPanel.new(); detail.talent = talent; detail.color = UIPalette.GOLD if talent["type"] == &"keystone" else meta["color"]; detail.can_increase = talent["rank"] < talent["max"] and _spent() < HeroData.TALENT_POINTS_TOTAL
			detail.position = Vector2(8,size.y-82); detail.size = Vector2(size.x-16,76); detail.decrease_requested.connect(_decrease_rank); detail.increase_requested.connect(_increase_rank); add_child(detail)
...
func _increase_rank(talent_id: StringName) -> void:
	...
	if talent.is_empty(): return
	var meta: Dictionary = HeroData.talent_sections()[talent["section"]]
	if talent["rank"] >= talent["max"] or _spent() >= HeroData.TALENT_POINTS_TOTAL or HeroData.PROGRESSION_LEVEL < meta["unlock"]: return
	talent["rank"] += 1; talent_rank_changed.emit(talent_id, talent["rank"]); _rebuild()


func _decrease_rank(talent_id: StringName) -> void:
	var talent := _find_talent(talent_id)
	if talent.is_empty() or talent["rank"] <= 0: return
	talent["rank"] -= 1; talent_rank_changed.emit(talent_id, talent["rank"]); _rebuild()
